...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (![self.slidingViewController.underRightViewController isKindOfClass:[YDFriendsViewController class]]) {
        YDSearchFriendsViewController *searchViewController = [[YDSearchFriendsViewController alloc] initWithNibName:nil bundle:nil];
        YDFriendsViewController *friendsViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"FriendsIdentifier"];
        friendsViewController.searchBar = searchViewController.searchBar;
        self.slidingViewController.underRightViewController  = friendsViewController;
...
- (void)setupNavigationBar {
	[self.navigationBar setBackgroundImage:[UIImage imageNamed:@"nav_home_bkg"] forBarMetrics:UIBarMetricsDefault];
